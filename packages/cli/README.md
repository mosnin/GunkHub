...
v0.19.0 — **New command: `afr policy scan|list|define|disable`.** Declarative policy over recorded runs, built to be a compliance gate — and built on the premise that **this tool cannot tell you that you are compliant.** It can tell you a forbidden operation WAS RECORDED, which is a fact. It can tell you it could not establish anything, which is the common answer. And in a narrow case it can tell you no violation appears in a set of runs it read end to end — a much smaller claim than it sounds, and the output states its own scope every time rather than trusting the reader to remember. **Exit codes: 0 no violation and every policy evaluable, 20 violation, 21 could not establish**, plus the shared 1/2/3/4 band. 20 wins over 21: a recorded breach is a positive fact and another policy going unevaluated does not make it less true, so a violation exits 20 even in a body with defects elsewhere. **There is no flag that turns 21 into 0, and the absence is the feature.** `afr budget check` has `--if-unavailable` because a breaker decision is a business tradeoff somebody may legitimately want to make; the output of a compliance scan gets attested to, and an operator who could turn "we could not look" into exit 0 would be manufacturing the attestation this whole feature exists against. `--if-unavailable`, `--accepted-risk` and `--fail-on` are all rejected as unknown options, and the test suite asserts that rather than trusting the help text. **Zero policies in scope exits 21, not 0 — a deliberate divergence from `afr budget check`,** which exits 0 when no budget governs a subject. A budget's absence is a business choice; a compliance scan over a subject with no policies attached that exits 0 is a green pipeline certifying nothing, and "we deleted the policies in a bad migration" would be indistinguishable from "we passed". It gets its own verdict, `no_policy_governs_this_subject`, so the two are distinguishable on every surface and not just in the exit code. `afr policy list` never exits 0 either: "we have policies" is not "we obeyed them". **The output names what it did not look at.** The three outcome counts print on one line, always — a satisfied figure travelling alone is the attestation figure with every safeguard stripped off. The retention horizon prints when one is configured, because a compliance report going clean BY ELAPSED TIME (ADR-001 purges) is the failure that will happen without anyone deciding it. An `environment` subject prints as a self-report, since ADR-002 leaves the label client-supplied and unvalidated. OTel-derived runs print an ordering caveat. And the most common `not_evaluable` kind today is `instrumentation_undeclared`: the SDK's HTTP and tool builders are called by hand with no interception, so a run that egressed through a plain `fetch()` is indistinguishable from one that touched no network — the remedy printed is to DECLARE complete instrumentation, not to build a cleverer evaluator. **`--deny-tools` / `--deny-hosts` with no value is not shorthand for a long list**: it is the deny-the-operation form, the only one whose findings survive Event Log Rule 3, because the event type outlives externalization when the tool name does not — and `afr policy list` says which coverage each rule has. `define`/`disable` are PRIVILEGED and audited server-side; `--rationale` is required on define because it travels into every finding this policy produces, and `--reason` on disable because turning off a compliance control is the act most likely to be asked about later. **There is deliberately no `delete`:** a policy that judged recorded runs is part of how they were judged, and removing the row would leave past findings pointing at nothing. Server support for `/api/v1/policies/**` is not wired yet; until it is, these exit 3. Requires contracts >= 0.26.0 / SDK >= 0.29.0.

v0.18.0 — **New command: `afr budget check|list|trip|reset`.** Budget circuit breakers from the command line, built to be a deploy gate. **`--if-unavailable` DEFAULTS TO `deny`, and that default is the command.** A gate that passes when it cannot reach the breaker is not a gate: anybody who wants to ship past it causes a network error. `grace` honours a yes we ALREADY received for `--grace-ms` past its expiry and never invents one; `allow` passes without an answer and REQUIRES `--accepted-risk`, because you may weaken the breaker and you may not do it without a sentence somebody can find later — and the decision still reports as `allowed_without_answer`, never as headroom. Note the SDK makes the same choice REQUIRED with no default at all: this command is a gate and can have a safe default, whereas a library embedded in someone's agent loop must not hand them a posture they did not pick. **Exit codes: 0 proceed, 10 tripped, 11 cannot establish**, plus the shared 1/2/4 usage/auth/server band. 10 wins over 11. **There is no flag that turns 11 into 0**, and the reason is the ADR-002 one: `--if-unavailable allow` reaches 0 when there is NO ANSWER (our outage), but an answer that says "undecidable" is a real reading of real data that does not support a conclusion — an approximate spend of 9,900 against a 10,000 cap means NOBODY KNOWS, and rounding that to a green light is the whole failure this feature exists against. A transport OUTAGE is fed to the policy rather than short-circuiting to exit 4, so an outage and a bad API key are not the same exit; auth and not-found still short-circuit, because those are misconfiguration and a gate that fails closed on a typo'd key is one nobody can debug. **Exactly one subject** (`--run` / `--agent` / `--project`) is required — there is deliberately no "whole org" default, because a gate whose subject is implicit changes meaning the day someone adds an org-wide budget, and it changes it in the direction that stops deploys. Reconciled and approximate spend print differently, every approximate figure prints its error bounds (and "unbounded" is one of the things a bound can be), and an UNDETERMINED breaker prints in its own register rather than as a quiet variant of ARMED. The decision sentence is composed by contracts, so no future edit here can phrase a decline as an outcome — and the help text says outright that "the agent was stopped" is not a fact this tool has. `trip`/`reset` are PRIVILEGED and audited server-side into the append-only admin audit log; `--reason` is required and never defaulted, because a manual trip has no meter reading behind it and the audit entry's only content is the sentence an operator wrote. The audit log id comes back and prints as the receipt. Server support for `/api/v1/budgets/**` is not wired yet; until it is, these exit 3. Requires contracts >= 0.25.0 / SDK >= 0.28.0.
