...
v0.29.0 — **Declarative policy: `PolicyPreflight`, `FlightReader.getPolicySnapshot()` / `getPolicyEvaluation()`, and the second thing this SDK does that is not recording.** A policy says a thing must not happen — "agent X may not call tool Y", "no run in env Z may egress to host H" — checked against what was actually recorded, plus a synchronous pre-flight the agent can ask before acting. Governed by ADR-009. **(0) A POLICY ENGINE INSIDE A RECORDER MUST NEVER REFUSE TO RECORD A VIOLATION, and this overrides everything else.** The breach is the most valuable event in the log: it is the run an engineer needs to open and the run a regulator will ask for, and a design where a policy hit causes the event to be dropped turns the product's most valuable record into its only missing one, silently, at the moment it matters most. So nothing in this feature sits on the ingest path, there is no `onViolation` hook and no `PolicyAction` — a policy cannot express "and then drop it" because there is nowhere to write it down — every one of the six preflight bands carries `recordRegardless: true` (the LITERAL type, in a total table whose value type is `true`, so a future edit cannot express otherwise), and `FORBIDDEN_SUPPRESSION_FIELDS` (`dropEvent`, `suppressViolation`, `doNotRecord`, ...) is REFUSED OUTRIGHT at the wire rather than softened into an incomplete verdict — a deployment asking this SDK to help a breach go unrecorded is not a bad scan, it is a compromised backend. **(1) THE EVIDENTIAL ASYMMETRY RUNS THE OPPOSITE WAY FROM SPEND.** `budgets.ts` had to make headroom hard to establish; here `violated` is PROVABLE (an event shows the forbidden call — a positive fact in an append-only log, and one event establishes it) and `satisfied` is essentially UNPROVABLE (the claim that the act appears nowhere). Two things make that bite: Event Log Rule 3 externalizes payloads over 10 KB, so a policy about tool NAMES cannot be evaluated over exactly the runs with the largest payloads; and — the deeper limit — **`Events.toolCall` and `Events.httpRequest` are MANUAL BUILDERS with no interception behind them, so a run that egressed through a plain `fetch()` is byte-identical, to this engine, to one that touched no network at all.** A complete read of an incomplete recording proves nothing about the world, and no field obtainable BY READING would change that, because the missing fact was never written. So the fact is CLAIMED, not inferred, using ADR-008's `{ declared: 'none' }` device: `PolicyCoverageProof` requires a `CompleteInstrumentationClaim` — an agent version asserting "all my egress goes through the recorder", with the mechanism written down so the claim is falsifiable — and `{ claims: 'undeclared' }` is not assignable, which makes `satisfied` UNREACHABLE for every agent in the product today. That is the honest answer, exactly as `provably_under` is unreachable for a counter-backed budget, and `violated` is unaffected: a recorded forbidden call proves a breach regardless of what else went unrecorded. `SatisfactionLicence` is a ONE-MEMBER UNION, so there is exactly one way to establish satisfaction and the extension point is a single greppable line; the proof carries five literal-typed fields (`logReadComplete: true`, `payloadsAllReadable: true`, `runIsTerminal: true`, `crossOrgRowsSkipped: 0`, `forbiddenOperationsFound: 0`) plus that claim, so a truncated read, an in-flight run, an externalized field, contaminated rows or a found violation each make it UNSPELLABLE rather than merely rejected. **One capability falls out of the same reasoning in the other direction:** when a rule denies the OPERATION itself (`deniedTools` absent — "may not call any tool"), the event TYPE decides it and the type survives externalization, so some policies are evaluable over exactly the runs that defeat others. The boundary is `undefined`, never "a short list", and a type-alone proof against a targeted rule is reported as `type_alone_against_a_targeted_rule` — widening it manufactures proofs. **(2) `not_evaluable` MAY NEVER RENDER AS `satisfied`, BECAUSE SOMEBODY WILL ATTEST TO IT.** A wrong spend figure costs money and is caught by reconciliation; a compliance surface reporting "no violations" when it could not look does not stay inside the building. The three outcomes share NO field but the discriminant (`violatedPolicyId` / `satisfiedPolicyId` / `undecidedPolicyId`), so `o.satisfiedPolicyId ?? o.undecidedPolicyId` is a compile error. There is no one-word name for the good outcome anywhere — the verdict is `no_violation_and_every_policy_was_evaluable`, long on purpose — and `clean`, `compliant`, `passed`, `attested` and `certified` are refused at the wire, in field names AND in prose, because a free-text "run was compliant" is the one channel every field check misses. **And the count is how the vocabulary escapes the type system:** `satisfiedCount: 12` in a response body carries none of the literals, none of the claim and none of the prose, so `PolicyOutcomeCounts` requires all three counts as fields — a bare satisfied figure is unconstructible — and `satisfiedCount`, `passRate` and `complianceRate` are refused on the wire. **(3) WE ADVISE, WE DO NOT PREVENT.** `PolicyPreflight.check()` is synchronous, does NO I/O at all (a prohibition is decidable in the client; only spend needs the server), never throws, and returns one of six field-disjoint bands — four mean proceed, for four different reasons, so an org whose answers are 100% `proceeded_without_policy_answer` cannot look identical to one with a working preflight. `unavailablePolicy` is REQUIRED with no default and the permissive arms demand a prose `acceptedRisk`. There is `wasAdvisedAgainstBySdk` and deliberately no `wasCallPrevented`. A matching policy advises against even on a STALE or TRUNCATED listing — a prohibition does not lapse by ageing — while a truncated listing can never establish that nothing forbids an act, since the policy that does is as likely to be in the unread tail. And the preflight decides a DESCRIPTION, not an act: nothing binds the name passed to `check()` to the call subsequently made, which is why the recorded run is the only thing that can ever contradict it. **Extraction and matching are ONE function, because separating them produced a live false all-clear.** `matchRecordedEventAgainstPolicy(event, policy)` takes a whole recorded event and answers in four bands. The obvious factoring — a reader handing a string to a matcher — was the factoring, and it failed: the reader accepted `host` and `hostname` as well as `url`, the matcher only parsed full URLs, and an `http.request` carrying `host: "evil.example"` was examined, counted as a present and fully legible deciding field, matched against nothing, and returned `satisfied`. A recorded, inline, legible forbidden egress reported as compliant. Teaching the matcher a second spelling would have left two places that must agree about what a host is, so the duplicate normaliser was DELETED: `hostForMatching` accepts both a URL and a bare authority, the pre-flight and the retrospective evaluator go through identical code, and the raw-value reader is deliberately unexported — a caller holding a raw value would have to decide for itself what "no match" means, and the honest answer depends on whether the value normalised. `RuleMatch` is four-valued so `undecidable` is not `permitted_by_this_rule` (a value present but uninterpretable is evidence we could not read, never evidence of compliance — new kind `deciding_field_unreadable`) and `not_relevant` is not either (a run of purely irrelevant events has not been shown to comply). **The same predicate takes the POLICY, not the rule**, for the same reason one level up: an evaluator taking a `PolicyRule` structurally cannot see `enabled`, while `actIsForbiddenBy` took the whole policy and did check it — two halves of one predicate asking different questions about the same subject, so a DISABLED policy produced violations. That direction is worse than the first: a false all-clear is believed, but A FALSE VIOLATION IS ACTED ON — somebody rolls back on a rule that was explicitly turned off. `policyGoverns` is now the single reading of enablement and interpretability, read by both halves, and a policy that governs nothing is `not_evaluable` — never a violation, and never an all-clear, so a failing scan cannot be made to pass by switching its policies off. Server support for `/api/v1/policies/**` is not wired yet; until it is, these surface `not_found`. Additive; no existing export changed. Requires contracts >= 0.26.0.

v0.28.0 — **Budget circuit breakers: `BudgetGuard`, `FlightReader.getBudgetSnapshot()`, and the first thing this SDK does that is not recording.** The backend can compute that a breaker is tripped; only the SDK sits in the agent's process and can DECLINE TO PROCEED. That changes the risk profile completely — a recording bug loses data, an enforcement bug stops a company's agents from working — and three invariants follow from it. **(1) WE RECORD, WE DECLINE, WE DO NOT STOP.** The SDK returns a decision; a caller that reads it and proceeds anyway is not a bug to defend against, it is the only thing that could ever have happened, because this is a library inside someone else's loop and not a supervisor above it. So "the breaker is tripped" and "the SDK declined" are facts we own, "the agent halted" is not, and NO type, field or helper can express the third: there is `wasDeclinedBySdk` and deliberately no `wasAgentStopped`, because there is no honest implementation of one. `FORBIDDEN_ENFORCEMENT_CLAIM_FIELDS` enumerates the vocabulary (`enforced`, `halted`, `blocked`, `prevented`, ...) and the wire gate REFUSES a body carrying any of them — the same treatment `BANNED_SUSPICION_HEADLINES` gets, and for the same reason: every other barrier stops a CONSUMER over-claiming by forgetting something, and a transmitted field lets the PRODUCER do it in one keystroke. A field named `enforced` becomes a line in an incident review and eventually a compliance claim nobody can support. **(2) "I WAS TOLD YES" AND "I COULD NOT ASK" ARE DIFFERENT TYPES.** A breaker that fails open when the server is unreachable is not a breaker (anyone wanting to bypass it causes a network error, the easiest condition in computing to arrange); one that fails closed halts honest work during an outage of OURS. Both costs are real and they fall on different people, so `BudgetGuard`'s `unavailablePolicy` is a REQUIRED constructor argument with NO DEFAULT — `{ onUnavailable: 'deny' | 'grace' | 'allow' }` — and the two permissive arms additionally require a prose `acceptedRisk`, so you may weaken the breaker but not silently. `grace` is the one to reach for: it honours a POSITIVE ANSWER WE ACTUALLY RECEIVED for a bounded window past its stated expiry, and never invents one. **Whatever is chosen, the outcome is never laundered**: `BudgetDecision` has SIX bands sharing no field but the discriminant, and FOUR of them mean proceed for four different reasons — `allowed_breaker_armed` (enforcement working), `allowed_no_budget_governs` (enforcement absent), `allowed_within_grace` (degraded), `allowed_without_answer` (off). A single `allowed: boolean` would have made an org that quietly lost every budget look identical on every dashboard to one with cost control. `mayProceed` is a TOTAL map over the bands so a seventh cannot ship unclassified, and it fails closed on an unrecognised one. **(3) AN APPROXIMATE FIGURE IS NOT A SMALL EXACT ONE.** ADR-002 is explicit that `usage_counters` is approximate and NOT billing-grade — single-unit increments flush ~1-in-10 and scale 10x — so it is a sampled estimator, wrong in EITHER direction. A hard limit built on it is a lie unless the approximation is in the type, and the stakes are concrete: an approximate $99 against a $100 cap and an exact $99 against a $100 cap are not the same claim, and the difference decides whether someone's production agents stop. `ReconciledSpend` carries `reconciledAmount`, `ApproximateSpend` carries `estimatedAmount`; there is NO `amount`, no `spent`, no `total`, so `spend.amount >= limit.limitAmount` does not compile against either and the only comparison is `compareSpendToLimit`, which is THREE-VALUED. The error bounds are `number | null` and **`null` is not `0`**: `null` means unbounded and makes `provably_under` UNREACHABLE, while `0` would be the claim that the figure cannot be low — a claim `usage_counters` cannot support. The two directions are tightened asymmetrically on purpose: `provably_at_or_over` needs the estimate MINUS its overstatement bound to still clear the cap, `provably_under` needs the estimate PLUS its understatement bound to still leave room, and the middle is `not_decidable` — never rounded toward either, and certainly never toward "under". A breaker cannot be `armed` on a figure that only compares `not_decidable`; `snapshotClaimContradictions` reports `armed_on_undecidable_spend`, driven by a TOTAL table over the claim kinds so a new self-claim cannot ship unaudited. **The cheap ask is designed, not hoped for.** An agent may check before every model call, and a breaker nobody can afford to consult is one that gets commented out: `BreakerSnapshot` carries the WHOLE governing set in one call, the SERVER states how long its answer is good for (`freshUntil` — only it knows the spend rate and the distance to the cap), the answer can ride back on an ingest response the agent already pays for (`absorbSnapshot`), and `check()` is SYNCHRONOUS with no I/O. The client caps the shelf life at `MAX_BREAKER_ANSWER_FRESHNESS_MS` whatever the server says, because an unbounded shelf life is a bypass. **Nothing in the guard throws** — an exception in an enforcement path is an enforcement outcome nobody chose, and inside somebody's `try/catch` it is the permissive one — so no snapshot, a malformed snapshot, an expired snapshot, a clock that throws and a proxy that raises on property access all land on `declined_no_answer` under `deny`; `tests/unit/budget_fail_closed.test.ts` induces each. The one thing that does throw is the CONSTRUCTOR, on a misconfigured policy, at wiring time rather than at decision time. A REFUSED snapshot is never stored and never displaces or extends a good one. Server support for `GET /api/v1/budgets/snapshot` is not wired yet; until it is, `getBudgetSnapshot` surfaces `not_found` and a `deny`-policy guard declines — which is the correct behaviour for a breaker whose server is not there. Adds `postV1` to the shared v1 client for the CLI's privileged trip/reset; the read surface and `packages/mcp` are unaffected. Additive; no existing export changed. Requires contracts >= 0.25.0.

v0.27.0 — **Cross-run causality: `FlightReader.getCausalTrace()`, and a new structural invariant.** Runs were isolated islands; this walks the RECORDED cross-run graph — up to what produced a run's input, down to what consumed its output, or outward to the whole connected component for an incident. **Two separations, both structural rather than advisory.** (1) RECORDED vs INFERRED: `RecordedCausalEdge` and `SuspectedLink` are mutually unassignable, share no text field, and a suspicion carries NO DIRECTION of any name — so it is not merely marked do-not-walk, it is UNWALKABLE, because there is no field a traversal could follow. Two runs adjacent in time, sharing a session, or touching the same resource are not thereby causally linked, and an arrow is the most persuasive object this product draws: nobody reads a confidence badge next to one. (2) **ENDED vs LOST, which is what this cycle exists for.** "The origin is run X" and "we lost the trail at run X" are opposite claims about the same run id — the first ends an investigation, the second says it is unfinished — and the second is the COMMON case in production because the SDK may never have recorded the edge. A flag would not have worked: `{ runId, isOrigin }` fails the moment a renderer prints `Origin: ${t.runId}` and reads the flag only to pick a colour, which is what every renderer does. So `RecordedOrigin` and `LostTrail` are two types sharing NO FIELD but the discriminant — `originRunId`/`hopsToOrigin` vs `lastReachedRunId`/`hopsBeforeLoss` — so no template can render one as the other, and the two hop counts are genuinely different quantities (a total vs a FLOOR: the `null`-versus-`0` lesson in numeric form). An origin can only be built from an `OriginProof` whose `inboundReadComplete` is the LITERAL TYPE `true` and whose `inboundEdgesFound` is the LITERAL TYPE `0`, so a truncated or non-empty adjacency read producing an "origin" is a compile error, not a convention. **A third disposition, `CycleReEntry`, was added after review**: retry loops and supervisor patterns are ordinary, a walk that closes one read everything it meant to, and filing that as `LostTrail` would state something false about the SCAN and mean no retry chain could ever exit 0. It shares no field with the other two either (`reEnteredRunId`/`hopsToReEntry`), and `COMPLETE_TERMINI` is a total map over the discriminants so a fourth band cannot default to "complete" by omission. **Fan-in was deliberately NOT made a fourth band**: a convergence is an ordinary interior node the walk continues through, so a `FanIn` terminus would duplicate `edgesInto` and double-count against the branch termini describing the same walk — it is exposed as `convergencePoints()`, and a walk that STOPS at a convergence is a `LostTrail` with kind `convergence_not_followed`, which correctly forces exit 11. **The artifact question is answered in the contract and enforced**: a matching SHA-256 found by joining two runs' artifact rows is `shared_resource` (a suspicion), and only a citation from the CONSUMER's own log with `role: 'consumed'` makes an `artifact_handoff` — `edgeIncoherences` reports `artifact_handoff_not_cited_by_consumer` otherwise. The checksum is what makes that inference feel like proof, which is what makes it the most dangerous one here. **The record lives in the immutable log, never a side table**: every citation is an event, an artifact the run's own log records handling, or a run field, because a mutable side table launders a lost edge into an absence indistinguishable from "there was no edge" — the ended-versus-lost distinction defeated by its own storage. Any edge table is a derived, rebuildable index. `RECORDED_CAUSAL_EDGE_KINDS` is exported and the five spellings are FROZEN as of this version, because `kind` is a stored value. **Absence of an edge is not absence of causation**: `EdgeAdjacency` is three-valued, so "there is no edge here" and "I do not know whether there is" are separate answers. **The gate refuses on ten grounds**, five carried over and five new; the two worth naming are `unproven_origin` (a lost trail wearing an origin's clothes — the one output that tells an operator to stop looking) and an EMPTY `termini` list, which is a vacuity: `termini.every(t => t.terminus === 'recorded_origin')` is TRUE on an empty array, so a server reporting no frontiers at all would read as a fully-traced graph to the most obvious check anyone would write. **Four defects were then found by attack, and they are one class — the class this repo named last cycle: verifying that a claim is PRESENT and INTERNALLY WELL-FORMED is not verifying that it AGREES WITH THE DATA BESIDE IT.** All four were decidable at no extra request from the edge set already in hand, and all four produced `complete: true`. (a) An edge with `recordedBy: []` certified as `chain_recorded` — and the sharpest detail is that an edge with ONE BAD citation was REJECTED, so **no evidence graded better than bad evidence**. (b) A `RecordedOrigin` whose proof asserted an empty adjacency set while the same traversal carried an edge into that run: a lost trail certifying as an origin, the exact distinction the feature exists to preserve, defeated by a self-reported claim nobody audited. (c) A cycle of length >= 2 invisible to every gate, because only `self_loop` is decidable from a SINGLE edge and every other rule saw one edge at a time. (d) `cyclePath: ['A','GHOST','A']` with no such edges — `unclosed_cycle` validated SYNTACTIC closure only, and a cycle is a COMPLETING disposition. **The repair is one thing, not four**, on the `REPORT_COLLECTIONS` precedent: `traversalClaimContradictions` builds the graph ONCE and runs `CLAIM_AUDITS`, a TOTAL `Record` over `CausalClaim`, so a new self-claim is a compile error until it has an audit — a per-claim check is a list, and a list fails the way these bugs failed. "Adjacent" follows the walk's DIRECTION (producers upstream, consumers downstream, both for a component), because auditing the wrong side would reject every honest downstream trace. The empty-citation case additionally lives in the per-EDGE primitive, because the SDK gate, the backend fold and the web adapter each had their own emptiness check, all three caught it, and that redundancy is precisely what hid the hole in the primitive everything new reaches for. **Reviewed again, and a barrier was upgraded from runtime to unrepresentable.** A `component` walk closed BOTH sides, so every run with any lineage has an edge touching it and the origin audit rejected every origin a component scan could ever emit — the claim was not usually wrong, it was never satisfiable, yet it compiled. A shape that exists and can never be honest is the WEAK form of a barrier and the form this contract argues against everywhere else. `component` is therefore no longer a direction: `CausalDirection = DirectedWalk | 'component'`, `TerminusFor<D>` hands a component walk `ComponentTerminus = CycleReEntry | LostTrail` with NO ORIGIN ARM, and `getCausalTrace` infers `D` from the caller's own `direction` — so `getCausalTrace({ direction: 'component' })` returns a traversal whose `termini[0].originRunId` DOES NOT COMPILE, for every consumer, since the read path is the only way anyone gets one. A cycle and a lost trail both stay legal, so a component walk can still report a finished trace; it just cannot say where a chain started, which is what an `upstream` walk is for. The wire check stays and must — a JSON body is not typechecked by anyone — but it now reports `component_origin_claimed` rather than the generic adjacency complaint, because an engine author told "your edge set disagrees" goes looking in the wrong place. **`sequenceNumber: 0` is now refused** (`not_a_sequence_number`): Event Log Rule 4 starts sequences at 1, so `0` is a sentinel written where the real position was lost, and a sentinel drawn from the data's own domain reads as a measurement — this one would point an operator at the wrong end of a run's log. **The barrier then shipped INERT, and the one-token bug is less interesting than the proof that missed it.** `CausalTraceParams<D>` declared `direction: CausalDirection` instead of `direction: D`, so nothing in the parameter object mentioned `D`; `getCausalTrace<D>` had no site to infer from, `D` fell back to its default, and `TerminusFor<CausalDirection>` widened straight back to the full `ChainTerminus` — origin included. It also left `getCausalTrace<'upstream'>({ direction: 'component' })` legal, which is the conflation spelled out in one call. **A PHANTOM TYPE PARAMETER FAILS OPEN AND SILENTLY**: it does not error, it does not narrow, and every test written against an EXPLICIT type argument keeps passing — which is exactly what the original proof did. It read `.originRunId` under `@ts-expect-error`, and that errors on the ordinary three-band union whether or not `D` infers, so the proof and the hole were compatible. The proof is now POSITIVE and only satisfiable by inference (the result of `getCausalTrace({ direction: 'component' })`, with no type argument, must be assignable to `ComponentTraversal`), and it was verified by reverting the fix and watching it go red. A generic-hygiene sweep was added alongside it, asserting that each parameterised type produces MUTUALLY NON-ASSIGNABLE types for two different arguments — the property a phantom parameter destroys — so the class is pinned and not just this instance. `packages/cli/src/apiClient.ts`'s wrapper is generic for the same reason: declaring the default there would have re-opened the hole one layer out. Additive; no existing export changed. Requires contracts >= 0.24.0.
